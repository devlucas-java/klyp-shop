...
func InitOrderModule(db *gorm.DB, log *logger.Logger, jwtService *jwt.JWTService) chi.Router {
	...
	orderService := service.NewOrderService(log, orderRepository, userRepository, addressRepository, productRepository, mapper.NewOrderMapper())
	...
	orderItemRouter := router.NewOrderItemRouter(orderItemHandler)
